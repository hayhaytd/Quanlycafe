...
public class BillDAO extends CrudDAO<Bill, Long> {
    ...
    //chuathanhtoan # null: "SELECT * FROM Bills WHERE CardId=? AND Status=0";
    final String SELECT_BY_IDSTATUS0_SQL = "SELECT * FROM Bills WHERE CardId=? AND Status=0";
    // lọc từ ngày đến ngày trong Bnagr Bills trong sql:
...
// Hàm tìm từ ngày đến ngày trong bảng Bills trong sql:
public List<Bill> findByTimeRange(Date begin, Date end) {
    ...
    // Hàm lấy ngày cho checkin/checkout
...
// Hàm tìm id max trong bảng Bills trong sql:    
